WITH bedrock_usage AS (
  ...
  FROM
    cur
  WHERE
      (line_item_product_code like '%Bedrock%' or product_product_name like '%Bedrock%')  AND (line_item_usage_type LIKE '%token%' OR line_item_line_item_description LIKE '%Token%')
),
sums as (
SELECT
  model_name,
...
FROM
  bedrock_usage
GROUP BY
  model_name, line_item_resource_id, resource_tags_user_dept, month
ORDER BY
  request_count DESC
)
...
WITH bedrock_usage AS (
  SELECT
    ...
    product_product_name,
    CASE
      WHEN line_item_usage_type LIKE '%input-token%' THEN 'Input' --no S! 
      WHEN line_item_usage_type LIKE '%output-token%' THEN 'Output'
      ELSE 'Other'
    END AS token_type,
    line_item_usage_amount,
    line_item_unblended_cost,
...
SELECT DISTINCT
  line_item_resource_id,
  month,
  SUM(line_item_unblended_cost) OVER (PARTITION BY line_item_resource_id, month) AS total_cost
FROM
  bedrock_usage
ORDER BY
  line_item_resource_id, 
